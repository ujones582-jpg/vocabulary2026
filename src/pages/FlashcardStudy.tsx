    ...
    <div className="min-h-screen flex flex-col max-w-md mx-auto">
        ...
        <button onClick={() => setFlipped(!flipped)} className="w-full aspect-[3/4] max-h-[400px] perspective-1000">
          <div className={`relative w-full h-full transition-transform duration-500 preserve-3d ${flipped ? "rotate-y-180" : ""}`}>
            ...
            <div className="absolute inset-0 backface-hidden rotate-y-180 bg-primary rounded-2xl card-shadow flex flex-col items-center justify-center p-8 text-center">
              <span className="text-xs uppercase tracking-widest text-primary-foreground/60 font-medium mb-4">Definition</span>
              <p className="text-lg font-semibold text-primary-foreground leading-relaxed mb-4">{word.definition}</p>
              {word.example && (
      ...
      <div className="px-6 pb-8 space-y-3">
        <div className="flex gap-3">
          ...
          {isLast ? (
            <button onClick={handleFinish} className="flex-1 py-3.5 rounded-lg bg-primary text-primary-foreground text-sm font-medium transition-all active:scale-[0.97] flex items-center justify-center gap-2">
              <CheckCircle2 className="w-4 h-4" /> Take Quiz
            </button>
          ) : (
            <button onClick={handleNext} className="flex-1 py-3.5 rounded-lg bg-primary text-primary-foreground text-sm font-medium transition-all active:scale-[0.97] flex items-center justify-center gap-2">
